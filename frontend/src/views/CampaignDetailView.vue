  ...
  <div class="space-y-6">
    ...
    <div v-else-if="campaign" class="space-y-6">
          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="flex items-start justify-between">
              <div>
                ...
                <div class="flex items-center text-sm text-gray-600">
                  <span class="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-xs font-medium mr-2">
            ...
            <div class="space-y-3">
              ...
              <div class="flex justify-between text-sm text-gray-600">
                <span>Goal: ${{ formatNumber(campaign.target_amount) }}</span>
            ...
            <div v-if="campaign.status === 'active' && authStore.isAuthenticated">
              <button @click="showDonateModal = true" class="btn-primary w-full">
                Donate Now
              </button>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="card text-center">
          <div class="text-2xl font-bold text-green-600 mb-1">${{ formatNumber(stats?.total_donated || 0) }}</div>
          <div class="text-gray-600">Total Donated</div>
        </div>
        <div class="card text-center">
          <div class="text-2xl font-bold text-purple-600 mb-1">{{ stats?.days_remaining || 0 }}</div>
    ...
    <div v-if="showDonateModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-bold mb-4">Donate to {{ campaign?.title }}</h3>
        <p class="text-gray-600 mb-4">Donation functionality will be implemented soon!</p>
        <button @click="showDonateModal = false" class="btn-primary w-full">
          Close
        </button>
      </div>
    </div>
